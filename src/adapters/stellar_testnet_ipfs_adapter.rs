...
impl StorageAdapter for StellarTestnetIpfsAdapter {
    ...
    async fn store_new_item(&self, item: &Item, is_new_dfid: bool, creator: &str) -> Result<AdapterResult<String>, StorageError> {
        if is_new_dfid {
            // Step 1: Mint NFT for new DFID
            let nft_tx_hash = self.stellar_client
                .mint_nft(&item.dfid, creator, None)
                .await
                .map_err(|e| StorageError::WriteError(format!("Failed to mint NFT on Stellar: {}", e)))?;

            tracing::info!("🎨 NFT minted for new DFID: {} (TX: {})", item.dfid, nft_tx_hash);

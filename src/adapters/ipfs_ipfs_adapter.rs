...
impl IpfsIpfsAdapter {
    pub fn new() -> Result<Self, StorageError> {
        ...
        let ipfs_client = if let (Ok(api_key), Ok(secret)) =
            (std::env::var("PINATA_API_KEY"), std::env::var("PINATA_SECRET_KEY")) {
...
impl StorageAdapter for IpfsIpfsAdapter {
    ...
    async fn sync_status(&self) -> Result<SyncStatus, StorageError> {
        ...
        let node_info = self.ipfs_client.node_info().await.unwrap_or_else(|_| "unknown".to_string());

        Ok(SyncStatus {
            adapter_type: AdapterType::IpfsIpfs,
            is_synced: ipfs_connected,
            pending_operations: 0,
            last_sync: if ipfs_connected { Some(Utc::now()) } else { None },
            error_count: 0,
            details: {
                let mut details = HashMap::new();
                details.insert("implementation_status".to_string(), serde_json::Value::String("production".to_string()));
                details.insert("ipfs_node".to_string(), serde_json::Value::String(node_info));

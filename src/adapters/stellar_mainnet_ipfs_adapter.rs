...
use crate::stellar_client::{StellarClient, StellarNetwork, MAINNET_IPCM_CONTRACT};
...
impl StellarMainnetIpfsAdapter {
    ...
    pub fn new_with_config(config: Option<&AdapterConfig>) -> Result<Self, StorageError> {
        ...
        let mut stellar_client =
            StellarClient::new(StellarNetwork::Mainnet, contract_address.clone());
            ...
            config.and_then(|c| c.connection_details.custom_headers.get("nft_contract"))
        {
            ...
            tracing::warn!(
                "⚠️  Stellar Mainnet adapter: No NFT contract configured (NFT minting will fail)"
            );
        }
...
impl StorageAdapter for StellarMainnetIpfsAdapter {
    ...
    async fn store_new_item(
        ...
        if is_new_dfid {
            ...
            tracing::info!(
                "🎨 NFT minted for new DFID on MAINNET: {} (TX: {})",
                item.dfid,
                nft_tx_hash
            );

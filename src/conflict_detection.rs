...
use crate::types::{
    ...
    PendingReason, ResolutionStrategy, SuggestedAction, QualitySeverity
};
...
impl<S: StorageBackend> ConflictDetectionEngine<S> {
    ...
    fn detect_dfid_conflicts(&self, identifiers: &[Identifier]) -> Option<Vec<ConflictInfo>> {
        ...
        for (identifier_str, dfids) in dfid_mappings {
            if dfids.len() >= self.dfid_conflict_threshold {
                ...
                let affected_identifier = identifiers.iter()
                ...
                let conflict = ConflictInfo {
                    ...
                    description: format!("Identifier maps to {} different DFIDs: {:?}", dfids.len(), dfids),
    ...
    fn detect_duplicate_ambiguity(&self, identifiers: &[Identifier]) -> Option<Vec<ConflictInfo>> {
        ...
        if potential_matches.len() > 1 {
            let conflict = ConflictInfo {
                ...
                description: format!("Found {} potential duplicate matches with high similarity", potential_matches.len()),
                affected_identifiers: identifiers.to_vec(),
                suggested_resolution: Some(ResolutionStrategy::ManualReview),
                confidence: 0.75,
                metadata: {
                    let mut metadata = HashMap::new();
                    metadata.insert("potential_matches".to_string(), serde_json::json!(potential_matches));
                    metadata.insert("similarity_scores".to_string(), serde_json::json!(similarity_scores));
                    metadata
                },
            };
            conflicts.push(conflict);
            ...
            for (issue_type, severity, details) in quality_issues {
                let conflict = ConflictInfo {
                    ...
                    suggested_resolution: match severity {
                        QualitySeverity::Critical | QualitySeverity::High => Some(ResolutionStrategy::ManualReview),
                    ...
                    metadata: {
                        ...
                        metadata.insert("severity".to_string(), serde_json::json!(format!("{:?}", severity)));
    ...
    fn detect_cross_system_conflicts(&self, _identifiers: &[Identifier]) -> Option<Vec<ConflictInfo>> {
        // Placeholder for cross-system conflict detection
        // In a real implementation, this would check against external systems
        None
    }

    fn validate_identifier_quality(&self, identifier: &Identifier) -> Vec<(String, QualitySeverity, String)> {
    ...
    fn calculate_similarity_score(&self, _identifier: &Identifier, _existing_identifiers: &[Identifier]) -> f64 {
    ...
    fn can_auto_resolve(&self, conflicts: &[ConflictInfo]) -> bool {
        conflicts.iter().all(|conflict| {
            matches!(conflict.severity, ConflictSeverity::None | ConflictSeverity::Low) &&
            conflict.suggested_resolution.as_ref()
                .map_or(false, |res| matches!(res, ResolutionStrategy::AutoMerge | ResolutionStrategy::SkipProcessing))
        })
    }
    ...
    fn generate_resolution_actions(&self, conflicts: &[ConflictInfo]) -> Vec<SuggestedAction> {
        ...
        let has_critical = conflicts.iter().any(|c| matches!(c.severity, ConflictSeverity::Critical));
        let has_dfid_conflicts = conflicts.iter().any(|c| matches!(c.conflict_type, ConflictType::IdentifierDFIDMapping));
        let has_quality_issues = conflicts.iter().any(|c| matches!(c.conflict_type, ConflictType::DataQuality));
        ...
        for conflict in conflicts {
            if let Some(resolution) = &conflict.suggested_resolution {
                ...
                actions.push(SuggestedAction {
                    ...
                    description: format!("Resolve {} conflict: {}",
                        format!("{:?}", conflict.conflict_type).to_lowercase(),
                        conflict.description
                    ),
                    confidence: conflict.confidence,
                    automated: matches!(resolution, ResolutionStrategy::AutoMerge | ResolutionStrategy::SkipProcessing),
                });
            }
        }
    ...
    pub fn convert_to_pending_reason(&self, analysis: &ConflictAnalysisResult) -> Option<PendingReason> {
        if analysis.conflicts.is_empty() {
            return None;
        }

        // Convert the most severe conflict to a PendingReason
        let primary_conflict = analysis.conflicts.iter()
            .max_by(|a, b| {
        ...
        match primary_conflict.conflict_type {
            ConflictType::IdentifierDFIDMapping => {
                if let Some(identifier) = primary_conflict.affected_identifiers.first() {
                    let conflicting_dfids = primary_conflict.metadata
                    ...
                    Some(PendingReason::ValidationError("DFID mapping conflict with no identifiers".to_string()))
                }
            }
            ConflictType::DuplicateDetection => {
                let potential_matches = primary_conflict.metadata
                    .get("potential_matches")
                    .and_then(|v| v.as_array())
                    .map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect())
                    .unwrap_or_default();

                let similarity_scores = primary_conflict.metadata
            ...
            ConflictType::DataQuality => {
                let issue_type = primary_conflict.metadata
                    .get("issue_type")
                    .and_then(|v| v.as_str())
                    .unwrap_or("unknown")
                    .to_string();

                let severity = primary_conflict.metadata
                    .get("severity")
                    .and_then(|v| v.as_str())
                    .and_then(|s| match s {
            ...
            ConflictType::CrossSystem => {
                Some(PendingReason::CrossSystemConflict {
                    external_system: "unknown".to_string(),
                    conflict_type: primary_conflict.description.clone(),
                })
            }
            ConflictType::ValidationFailure => {
                Some(PendingReason::ValidationError(primary_conflict.description.clone()))
            }
        }
    }
}
...
## DFID Service Architecture (2025-01-31)

### Overview
DFID generation has been separated into an optional external service (Layer 1 of 4-layer architecture). The system supports dual-mode operation for zero-downtime migration.

### Architecture Modes
1. **Local Mode** (default): Uses embedded `DfidEngine` for backward compatibility
2. **Remote Mode**: Uses `DfidClient` when `DFID_SERVICE_URL` environment variable is set
3. **Hybrid Mode**: Both DfidEngine and DfidClient available, automatic fallback

### DFID Format
1. Format: `DFID-{YYYYMMDD}-{SEQUENCE}-{CHECKSUM}`
2. **YYYYMMDD**: Date of generation (8 digits)
3. **SEQUENCE**: 6-digit sequence number (000001-999999)
4. **CHECKSUM**: 6-character BLAKE3 hash (24-bit, hex)
5. Example: `DFID-20250131-000042-A7B2C3`

### DFID Service (Layer 1)
1. **Stateless Design**: Only atomic sequence counter in memory
2. **BLAKE3 Checksums**: Upgraded from 16-bit polynomial to 24-bit BLAKE3
3. **Redis Persistence**: Optional sequence counter persistence (survives restarts)
4. **REST API**: HTTP endpoints for generation, validation, batch operations
5. **Horizontal Scaling**: Multiple instances can share Redis backend

### DFID Client Integration
1. ItemsEngine and CircuitsEngine support optional DfidClient
2. `generate_dfid_internal()` method checks for client availability
3. Falls back to local DfidEngine if client unavailable or fails
4. Async methods: `create_item_with_generated_dfid()`, `split_item_with_generated_dfid()`
5. Configuration via `with_dfid_client()` builder method

### DFID Service API Endpoints
1. `POST /dfid/generate` - Generate single or multiple DFIDs
2. `POST /dfid/batch` - Bulk generation (up to 10,000)
3. `GET /dfid/:id/validate` - Validate format and extract metadata
4. `GET /health` - Health check with current sequence

### Deployment Configuration
1. **Local Development**: `docker-compose up` starts all services
2. **Railway Production**: Separate service `defarm-dfid-service`
3. **Environment Variables**:
   - `DFID_SERVICE_URL` - Enable remote DFID generation
   - `REDIS_URL` - Optional Redis for sequence persistence
4. **Migration**: Set DFID_SERVICE_URL to activate, unset to rollback

### Performance Characteristics
1. **Throughput**: >10,000 DFIDs/second per instance
2. **Latency**: <5ms per request (in-memory)
3. **Batch Generation**: Linear O(n) with minimal overhead
4. **Sequence Persistence**: Every 10 seconds to Redis

### Backward Compatibility
1. Old 16-bit checksums (4 hex chars) still validate
2. New 24-bit checksums (6 hex chars) generated by DFID Service
3. Mixed format supported during migration period
4. No breaking changes to Item Registry API

### Future Layers (Planned)
1. **Layer 2**: Item Registry (current API - metadata, deduplication)
2. **Layer 3**: Adapter Layer (blockchain/IPFS registration)
3. **Layer 4**: Index Service (DFID discovery and location tracking)

### Documentation References
- DFID Service README: `dfid-service/README.md`
- Deployment Guide: `DFID_SERVICE_DEPLOYMENT.md`
- Migration Guide: `MIGRATION_GUIDE.md`
- Verification Script: `scripts/verify_dfid_uniformity.sh`

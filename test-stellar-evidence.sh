...
# Test with Stellar Testnet to show NFT minting

echo "🚀 STELLAR BLOCKCHAIN EVIDENCE TEST"
echo "════════════════════════════════════════════════════════════════════════"
echo

# Check if Stellar is configured
if [ -z "$STELLAR_TESTNET_SECRET" ]; then
    echo "Setting up Stellar testnet configuration..."

    # Use test account (DO NOT USE IN PRODUCTION)
    export STELLAR_TESTNET_SECRET="SB3HE2YRGMU32T6WZBB3ORIGIFFIYRQ2BNVH4DDNSNEF3N5WYHDRM4NT"
    export STELLAR_TESTNET_PUBLIC="GBFGGJRKH6M2ZKJ6PNSCIEQPTZ5Z2BP5KDI7RHQQ4JLNLNAPFO5SKD7G"
fi

echo "Stellar Configuration:"
echo "  Network: Testnet"
echo "  IPCM Contract: ${STELLAR_TESTNET_IPCM_CONTRACT}"
echo "  NFT Contract: ${STELLAR_TESTNET_NFT_CONTRACT}"
echo

# Create test that uses Stellar adapter
cat > /tmp/stellar_test.rs << 'EOF'
use chrono::Utc;
...
echo "Running Stellar test..."
cd /Users/gabrielrondon/rust/engines
cp /tmp/stellar_test.rs tests/

cargo test test_stellar_evidence --test stellar_test -- --nocapture 2>&1 | grep -A 100 "STELLAR TESTNET EVIDENCE"